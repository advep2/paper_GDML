...
from GDML_paper_contour_2D_fig3 import contour_2D_fig3
...
path_out = "VHT_MP_US_plume_sims/paper_GDML_figs/fig3_paper/new_sims/revision2/"
...
def plot_MFAM_ax_nosig(ax,faces,nodes,line_width,line_color):
    nfaces = np.shape(faces)[1]
    for i in range(0,nfaces):
        if faces[2,i] == 1:     # face type >> lambda = const. (cyan)
            ax.plot(nodes[0,faces[0:2,i]-1],nodes[1,faces[0:2,i]-1],color=line_color,linewidth = line_width)
            
def plot_MFAM_ax_nosig_plume(ax,faces,nodes,face_geom,line_width,line_color):
    nfaces = np.shape(faces)[1]
    for i in range(0,nfaces):
        if faces[2,i] == 1 and face_geom[0,i] >= 1:     # face type >> lambda = const. (cyan)
            ax.plot(nodes[0,faces[0:2,i]-1],nodes[1,faces[0:2,i]-1],color=line_color,linewidth = line_width)
...
#    plot_cath_contours = 0
    ...
    # Cathode final boundary face for cases P3_fcat1962 (NOT USED HERE FOR P3 REFERENCE CASES)
    cath_final_bound_face_P3 = 1804
    cath_final_bound_face = [cath_final_bound_face_P3,cath_final_bound_face_P3]
    # Elements along cathode B line for VHT_US case P3 (python indeces)
    elems_cath_Bline_P3 = list(range(2222-1,2222-1-36,-2)) + list([2187-1]) + list(range(2184-1,2184-1-22,-2)) + list([2163-1])+list(range(2160-1,2160-1-38,-2))  # Elements along cathode B line for VHT_US P30 cases
    
    # Elements along cathode B line for VHT_US case P2G_fcat905 (python indeces)
    # elems_cath_Bline_P2 = [591, 589, 587, 584, 583, 581, 579, 576, 575, 572, 570, 568,
    #                        566, 565, 562, 560, 558, 556, 555, 553, 551, 548, 546, 545]  
    # Elements along cathode B line for VHT_US case P2G_fcat3198 (python indeces)
    ...
    # Elements along cathode B line for VHT_US case P3G_fcat1962 (python indeces)
...
#    last_steps      = 1000
    ...
    oldsimparams_sim = np.array([21,21,20,20,20,20,20,20,20,20,20,20],dtype = int)   
...
#    cont_ylabel = '$r$ (cm)'
    ...
    if plot_curr_ref == 1:
        ...
        # [fig1, axes1] = plt.subplots(nrows=1, ncols=3, figsize=(22.5,6))
        [fig1, axes1] = plt.subplots(nrows=1, ncols=3, figsize=(22.5,7))
    ...
    for k in range(0,nsims):
        ...
        if interp_MFAM_picM_plot == 1:
            if mean_vars == 1 and plot_mean_vars == 1:
                ...
                [zs_mp,rs_mp,dims_mp,nodes_flag_mp,xi_bottom_mp,xi_top_mp,eta_min_mp,
                 eta_max_mp,phi_mp,Te_mp,je_perp_mp,je_theta_mp,je_para_mp,je_z_mp,
                 je_r_mp,je_2D_mp,ji_x_mp,ji_y_mp,ji_z_mp,ji_2D_mp,j_r_mp,j_t_mp,
                 j_z_mp,j_2D_mp,ne_mp,Bfield_mp,Br_mp,Bz_mp,alpha_ano_mp,
                 alpha_ano_e_mp,alpha_ano_q_mp,alpha_ine_mp,alpha_ine_q_mp] = HET_sims_interp_MFAM_picM_plot(path_picM_plot,n_elems,n_faces,elem_geom,
    ...
    #    nu_i12_plot       = nu_i12_plot*1E-6
        ...
        if k == 0 or k == 1:
            ...
            elems_cath_Bline = elems_cath_Bline_P2
        elif k == 6 or k == 7:
            elems_cath_Bline = elems_cath_Bline_P3
    ...
    #    yticks_lim = rs[-1,-1]+1
        ...
        if plot_curr_ref == 1:
            ...
            if k == 0:
                ...
                zstext = 0.5
                rstext = 0.95
                text = text_title_1+r"$\tilde{\boldsymbol{\jmath}}_\mathrm{i}$ (Acm$^{-2}$)"
                axes1[ind1_1].text(zstext,rstext,text,fontsize = text_size,color='k',ha='center',va='center',transform=axes1[ind1_1].transAxes) 
                    
                # axes1[ind1_1].set_title(text_title_1+r"$\tilde{\boldsymbol{\jmath}}_\mathrm{i}$ (Acm$^{-2}$)", fontsize = font_size,y=1.02)
        ...
        #        lines_ticks_loc  = [(1.5,4.25),(3.4,4.25),(4.8,1.43),(7.56,7.4),(9.34,5.16),(8.45,0.6),(5.9,5.5),(4.0,6.4)]        
                ...
                lines_style       = '-'
                # if interp_MFAM_picM_plot == 1:
                #     var = np.copy(ji_2D_mp)
                #     if only_plume == 1:
                #         var[np.where(zs_mp < zs[0,int(xi_bottom)])] = np.nan
                #     [CS,CS2] = contour_2D (axes1[ind1_1],cont_xlabel, cont_ylabel, font_size, ticks_size, zs_mp, rs_mp, var, nodes_flag_mp, log_type, auto, 
                ...
                #     [CS,CS2] = contour_2D (axes1[ind1_1],cont_xlabel, cont_ylabel, font_size, ticks_size, zs, rs, var, nodes_flag, log_type, auto, 
                #                            min_val0, max_val0, cont, lines, cont_nlevels, auto_cbar_ticks, auto_lines_ticks,
                #                            nticks_cbar, nticks_lines, cbar_ticks, lines_ticks, lines_ticks_loc, cbar_ticks_fmt,
                #                            lines_ticks_fmt, lines_ticks_color, lines_style, lines_width) 
                var = np.copy(ji2D_plot)
                if only_plume == 1:
                    var[:,0:int(xi_bottom)] = np.nan
                [CS,CS2] = contour_2D_fig2 (axes1[ind1_1],cont_xlabel, cont_ylabel, font_size, ticks_size, zs, rs, var, nodes_flag, log_type, auto, 
                                        min_val0, max_val0, cont, lines, cont_nlevels, auto_cbar_ticks, auto_lines_ticks,
                                        nticks_cbar, nticks_lines, cbar_ticks, lines_ticks, lines_ticks_loc, cbar_ticks_fmt,
                                        lines_ticks_fmt, lines_ticks_color, lines_style, lines_width) 
                # Plot the MFAM if required
                if plot_MFAM_lines == 1:
                    # plot_MFAM_ax(axes1[ind1],faces,nodes,line_width)
                    # plot_MFAM_ax_nosig(axes1[ind1],faces,nodes,line_width_MFAM,color_line_MFAM)
                    plot_MFAM_ax_nosig_plume(axes1[ind1_1],faces,nodes,face_geom,line_width_MFAM,color_line_MFAM)
                    
                axes1[ind1_1].plot(points[:,0],points[:,1],'k-',linewidth = line_width_boundary,markersize = marker_size)
                axes1[ind1_1].plot(np.array([points[-1,0],points[0,0]]),np.array([points[-1,1],points[0,1]]),'k-',linewidth = line_width_boundary,markersize = marker_size)     
                # if plot_cath_contours == 1:
                #     axes1[ind1_1].plot(z_cath,r_cath,'ks',linewidth = line_width_boundary,markersize = marker_size)
                # # Plot the cathode magnetic line if required
                # if plot_cath_Bline == 1:
                #     axes1[ind1_1].plot(cath_Bline_z,cath_Bline_r,color=cath_Bline_color,linewidth = line_width_boundary,markersize = marker_size)
                # # Plot scan points if required
                # if plot_scan_points == 1:
                #     axes1[ind1_1].plot(z_scan,r_scan,color=scan_points_color,linewidth = line_width_boundary,markersize = marker_size)
        ...
        #        flag_start = 1
                flag_start = 0
                # if interp_MFAM_picM_plot == 1:
                #     stream = streamline_2D(axes1[ind1_1],zvec,rvec,dims,dims_mp,zs_mp,rs_mp,ji_z_mp,ji_x_mp,flag_start,start_points,
                #                            plot_start_points,streamline_density,streamline_width,streamline_color,arrow_size,
                #                            arrow_style,min_length)
                # else:
                #     stream = streamline_2D(axes1[ind1_1],zvec,rvec,dims,zs,rs,ji_z_plot,ji_x_plot,flag_start,start_points,
                #                                plot_start_points,streamline_density,streamline_width,streamline_color,arrow_size,
                #                                arrow_style,min_length)
                stream = streamline_2D(axes1[ind1_1],zvec,rvec,dims,zs,rs,ji_z_plot,ji_x_plot,flag_start,start_points,
                                               plot_start_points,streamline_density,streamline_width,streamline_color,arrow_size,
                                               arrow_style,min_length)
                axes1[ind1_1].set_ylim(0,ylim_axes)
                if only_plume == 1:
                    axes1[ind1_1].set_xlim(zs[0,int(xi_bottom)],xlim_axes)
                else:
                    axes1[ind1_1].set_xlim(0,xlim_axes)
    
                if ax_ticks_flag == 2:
                    if only_plume == 1:
                        # axes1[ind1_1].set_xticks(np.arange(zs[0,int(xi_bottom)],xlim_axes+1,1))
                        axes1[ind1_1].set_xticks(np.arange(zs[0,int(xi_bottom)],xlim_axes,1))
                    else:
                        # axes1[ind1_1].set_xticks(np.arange(0,xlim_axes+1,1))
                        axes1[ind1_1].set_xticks(np.arange(0,xlim_axes,1))
                    axes1[ind1_1].set_yticks(np.arange(0,ylim_axes+1,1))
                elif ax_ticks_flag == 1:
                    axes1[ind1_1].set_xticks(np.arange(0,zs[0,-1]+1,1))
                    axes1[ind1_1].set_yticks(np.arange(0,rs[-1,-1]+1,1))
                    
                axes1[ind1_1].tick_params(labelsize = ticks_size)
                # axes1[ind1_1].tick_params(labelsize = ticks_size,labelbottom=False)
                axes1[ind1_1].set_ylabel(cont_ylabel, fontsize = font_size)
                axes1[ind1_1].set_xlabel(cont_xlabel, fontsize = font_size)
                ...
                if plot_cath_Bline == 1:
                    ...
                    axes1[ind1_1].plot(cath_z,cath_r,color=cath_Bline_color,linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                    # axes1[ind1].plot(cath_Bline_z,cath_Bline_r,color=cath_Bline_color,linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                # Plot scan points if required
                if plot_scan_points == 1:
                    axes1[ind1_1].plot(z_scan,r_scan,color=scan_points_color,linestyle=scan_points_linestyle, linewidth = line_width_boundary,markersize = marker_size)
                # Plot singular magnetic line if required
                if plot_B0_line == 1:
                    axes1[ind1_1].plot(B0_line_nodez,B0_line_noder,color=B0_line_color,linestyle=B0_line_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                    
                # Plot cathode marker if required
                if plot_cath_contours == 1:
                    axes1[ind1_1].plot(1.05*z_cath,r_cath,linewidth = line_width_boundary,markersize = marker_size,marker = 's',color = cath_marker_color,markeredgecolor = cath_marker_edge_color)
                    
                # Plot vertical line if required
                if vline == 1:
                    if ind1_1 == 2 or ind1_1 == 3:
                        axes1[ind1_1].axvline(x=zP, linestyle='--',color=color_vline,linewidth = line_width_boundary)
                


            text = text_title+r"$-\tilde{\boldsymbol{\jmath}}_\mathrm{e}$ (Acm$^{-2}$)"
            axes1[ind1].text(zstext,rstext,text,fontsize = text_size,color='k',ha='center',va='center',transform=axes1[ind1].transAxes) 
            
            # axes1[ind1].set_title(text_title+r"$-\tilde{\boldsymbol{\jmath}}_\mathrm{e}$ (Acm$^{-2}$)", fontsize = font_size,y=1.02)
    ...
    #        lines_ticks_loc  = [(1.5,4.25),(3.4,4.25),(4.8,1.43),(7.56,7.4),(9.34,5.16),(8.45,0.6),(5.9,5.5),(4.0,6.4)]        
            ...
            if interp_MFAM_picM_plot == 1:
                ...
                if only_plume == 1:
                    var[np.where(zs_mp < zs[0,int(xi_bottom)])] = np.nan
                [CS,CS2] = contour_2D_fig2 (axes1[ind1],cont_xlabel, cont_ylabel, font_size, ticks_size, zs_mp, rs_mp, var, nodes_flag_mp, log_type, auto, 
                                       min_val0, max_val0, cont, lines, cont_nlevels, auto_cbar_ticks, auto_lines_ticks,
                                       nticks_cbar, nticks_lines, cbar_ticks, lines_ticks, lines_ticks_loc, cbar_ticks_fmt,
                                       lines_ticks_fmt, lines_ticks_color, lines_style, lines_width) 
            else:
                ...
                if only_plume == 1:
                    var[:,0:int(xi_bottom)] = np.nan
                [CS,CS2] = contour_2D_fig2 (axes1[ind1],cont_xlabel, cont_ylabel, font_size, ticks_size, zs, rs, var, nodes_flag, log_type, auto, 
                                       min_val0, max_val0, cont, lines, cont_nlevels, auto_cbar_ticks, auto_lines_ticks,
                                       nticks_cbar, nticks_lines, cbar_ticks, lines_ticks, lines_ticks_loc, cbar_ticks_fmt,
                                       lines_ticks_fmt, lines_ticks_color, lines_style, lines_width) 
            ...
            if plot_MFAM_lines == 1:
                # plot_MFAM_ax(axes1[ind1],faces,nodes,line_width)
                # plot_MFAM_ax_nosig(axes1[ind1],faces,nodes,line_width_MFAM,color_line_MFAM)
                plot_MFAM_ax_nosig_plume(axes1[ind1],faces,nodes,face_geom,line_width_MFAM,color_line_MFAM)
                
            axes1[ind1].plot(points[:,0],points[:,1],'k-',linewidth = line_width_boundary,markersize = marker_size)
            axes1[ind1].plot(np.array([points[-1,0],points[0,0]]),np.array([points[-1,1],points[0,1]]),'k-',linewidth = line_width_boundary,markersize = marker_size)     
            # if plot_cath_contours == 1:
            #     axes1[ind1].plot(z_cath,r_cath,'ks',linewidth = line_width_boundary,markersize = marker_size)
            # # Plot the cathode magnetic line if required
            # if plot_cath_Bline == 1:
            #     axes1[ind1].plot(cath_Bline_z,cath_Bline_r,color=cath_Bline_color,linewidth = line_width_boundary,markersize = marker_size)
            # # Plot scan points if required
            # if plot_scan_points == 1:
            #     axes1[ind1].plot(z_scan,r_scan,color=scan_points_color,linewidth = line_width_boundary,markersize = marker_size)
            # --- Plot the streamlines 
            # Define number of starting points for plotting the streamlines
            plot_start_points = 0
            start_points = np.zeros((0,2))
    ...
    #        flag_start = 1
            flag_start = 0
            if interp_MFAM_picM_plot == 1:
                stream = streamline_2D(axes1[ind1],zvec,rvec,dims_mp,zs_mp,rs_mp,-je_z_mp,-je_r_mp,flag_start,start_points,
                                       plot_start_points,streamline_density,streamline_width,streamline_color,arrow_size,
                                       arrow_style,min_length)
            else:
                stream = streamline_2D(axes1[ind1],zvec,rvec,dims,zs,rs,-je_z_plot,-je_r_plot,flag_start,start_points,
                                           plot_start_points,streamline_density,streamline_width,streamline_color,arrow_size,
                                           arrow_style,min_length)
            axes1[ind1].set_ylim(0,ylim_axes)
            if only_plume == 1:
                axes1[ind1].set_xlim(zs[0,int(xi_bottom)],xlim_axes)
            else:
                axes1[ind1].set_xlim(0,xlim_axes)

            if ax_ticks_flag == 2:
                if only_plume == 1:
                    # axes1[ind1].set_xticks(np.arange(zs[0,int(xi_bottom)],xlim_axes+1,1))
                    axes1[ind1].set_xticks(np.arange(zs[0,int(xi_bottom)],xlim_axes,1))
                else:
                    # axes1[ind1].set_xticks(np.arange(0,xlim_axes+1,1))
                    axes1[ind1].set_xticks(np.arange(0,xlim_axes,1))
                axes1[ind1].set_yticks(np.arange(0,ylim_axes+1,1))
            elif ax_ticks_flag == 1:
                axes1[ind1].set_xticks(np.arange(0,zs[0,-1]+1,1))
                axes1[ind1].set_yticks(np.arange(0,rs[-1,-1]+1,1))

            axes1[ind1].tick_params(labelsize = ticks_size)
            # axes1[ind1].set_ylabel(cont_ylabel, fontsize = font_size)
            axes1[ind1].set_xlabel(cont_xlabel, fontsize = font_size)
            axes1[ind1].tick_params(labelsize = ticks_size,labelleft=False)
            ...
            if plot_cath_Bline == 1:
                ...
                cath_z = elem_geom[0,elems_cath_Bline]
                cath_r = elem_geom[1,elems_cath_Bline]
                axes1[ind1].plot(cath_z,cath_r,color=cath_Bline_color,linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                # axes1[ind1].plot(cath_Bline_z,cath_Bline_r,color=cath_Bline_color,linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
            # Plot scan points if required
            if plot_scan_points == 1:
                axes1[ind1].plot(z_scan,r_scan,color=scan_points_color,linestyle=scan_points_linestyle, linewidth = line_width_boundary,markersize = marker_size)
            # Plot singular magnetic line if required
            if plot_B0_line == 1:
                axes1[ind1].plot(B0_line_nodez,B0_line_noder,color=B0_line_color,linestyle=B0_line_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                
            # Plot cathode marker if required
            if plot_cath_contours == 1:
                axes1[ind1].plot(1.05*z_cath,r_cath,linewidth = line_width_boundary,markersize = marker_size,marker = 's',color = cath_marker_color,markeredgecolor = cath_marker_edge_color)
                # axes1[ind1].plot(z_cath,r_cath,'ks',linewidth = line_width_boundary,markersize = marker_size)
                
            # Plot vertical line if required
            if vline == 1:
                if ind1 == 2 or ind1 == 3:
                    axes1[ind1].axvline(x=zP, linestyle='--',color=color_vline,linewidth = line_width_boundary)
    ...
    cbar_ax = fig1.add_axes([0.20, -0.03, 0.6, 0.040])  #  [left, bottom, width, height] in coordinates relative to figure
    ...
    fig1.tight_layout()
    if save_flag == 1:
        fig1.savefig(path_out+"fig3_rev2"+figs_format,bbox_inches='tight')
        fig1.savefig(path_out+"fig3_rev2"+figs_format2,bbox_inches='tight')
        plt.close(fig1)
