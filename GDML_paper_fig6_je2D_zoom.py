...
#    cont_ylabel = '$r$ (cm)'
    ...
    #        ax.clabel(CS2, CS2.levels, fmt=fmt_func_exponent_lines, inline=1, manual = lines_ticks_loc, fontsize=ticks_size_isolines)
            ax.plot(points[:,0],points[:,1],'k-',linewidth = line_width_boundary,markersize = marker_size)
            ax.plot(np.array([points[-1,0],points[0,0]]),np.array([points[-1,1],points[0,1]]),'k-',linewidth = line_width_boundary,markersize = marker_size)     
    ...
    #        flag_start = 1
            ...
            if only_plume == 1:
                ax.set_xlim(zs[0,int(xi_bottom)],xlim_axes)
            else:
                ax.set_xlim(1,xlim_axes)

            if ax_ticks_flag == 2:
    #            ax.set_xticks(np.arange(0,zs[0,-1]+1,2))
    #            ax.set_yticks(np.arange(0,rs[-1,0]+1,2))
                if only_plume == 1:
                    ax.set_xticks(np.arange(zs[0,int(xi_bottom)],xlim_axes+1,1))
                else:
                    ax.set_xticks(np.arange(1,xlim_axes+1,1))
                ax.set_yticks(np.arange(0,ylim_axes,1))
    ...
    #            ax.set_yticks(np.arange(0,rs[-1,-1],2))
            ...
            if plot_cath_Bline == 1:
                ...
                ax.plot(cath_z,cath_r,color=cath_Bline_color,linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                ...
                if k == 2 or k == 3 or k == 6 or k == 7:
                    # cath_z = np.append(elem_geom[0,elems_cath_Bline_P2],np.array([face_geom[0,cath_final_bound_face_P2-1]]),axis=0)
                    # cath_r = np.append(elem_geom[1,elems_cath_Bline_P2],np.array([face_geom[1,cath_final_bound_face_P2-1]]),axis=0)
                    ax.plot(cath_z_P2,cath_r_P2,color='g',linestyle='-.',linewidth = line_width_boundary,markersize = marker_size)
            # Plot scan points if required
            if plot_scan_points == 1:
                ax.plot(z_scan,r_scan,color=scan_points_color,linestyle=scan_points_linestyle, linewidth = line_width_boundary,markersize = marker_size)
            # Plot singular magnetic line if required
            if plot_B0_line == 1:
                ax.plot(B0_line_nodez,B0_line_noder,color=B0_line_color,linestyle=B0_line_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                
            # Plot cathode marker if required
            if plot_cath_contours == 1:
                ax.plot(z_cath,r_cath,'ks',linewidth = line_width_boundary,markersize = marker_size)
                
            # Plot vertical line if required
            if vline == 1:
                if ind1 == 2 or ind1 == 3:
                    ax.axvline(x=zP, linestyle='--',color=color_vline,linewidth = line_width_boundary)
    ...
    plt.tight_layout()
    if save_flag == 1:
        plt.savefig(path_out+"je2D_ref_complete_zoom"+figs_format,bbox_inches='tight')
        plt.close(fig1)
