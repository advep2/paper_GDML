...
path_out = "VHT_MP_US_plume_sims/paper_GDML_figs/fig1_paper/"
...
#    plot_cath_contours = 0
    ...
    cath_final_bound_face_P2C = 832
    # Cathode final boundary face for cases P3_fcat1962
    cath_final_bound_face_P3C = 1804
    # Cathode final boundary face for cases P3
    # cath_final_bound_face_P3 = 
    # cath_final_bound_face = [cath_final_bound_face_P2C,cath_final_bound_face_P2C,
    #                          cath_final_bound_face_P3C,cath_final_bound_face_P3C,
    #                          cath_final_bound_face_P2C,cath_final_bound_face_P2C,
    #                          cath_final_bound_face_P3C,cath_final_bound_face_P3C]
    cath_final_bound_face = [cath_final_bound_face_P3C,cath_final_bound_face_P2C]
    
    # Elements along cathode B line for VHT_US case P2G_fcat905 (python indeces)
    elems_cath_Bline_P2C = [591, 589, 587, 584, 583, 581, 579, 576, 575, 572, 570, 568,
                           566, 565, 562, 560, 558, 556, 555, 553, 551, 548, 546, 545]        
    # Elements along cathode B line for VHT_US case P3G_fcat1962 (python indeces)
    elems_cath_Bline_P3C = [1289, 1287, 1285, 1283, 1281, 1279, 1277, 1275, 1272, 1270,
    ...
    # Elements along cathode B line for VHT_US P3 reference cathode cases  
    elems_cath_Bline_P3 = list(range(2222-1,2222-1-36,-2)) + list([2187-1]) + list(range(2184-1,2184-1-22,-2)) + list([2163-1])+list(range(2160-1,2160-1-38,-2))              
    ...
    # B0_face_axis_ID = [B0_face_axis_ID_P2,B0_face_axis_ID_P2,
    #                    B0_face_axis_ID_P3,B0_face_axis_ID_P3,
    #                    B0_face_axis_ID_P2,B0_face_axis_ID_P2,
    #                    B0_face_axis_ID_P3,B0_face_axis_ID_P3]
    B0_face_axis_ID = [B0_face_axis_ID_P3,B0_face_axis_ID_P2]
...
#    last_steps      = 1000
    ...
    oldsimparams_sim = np.array([21,20,20,20,20,20,20,20,20,20,20,20],dtype = int)   
...
#    cont_ylabel = '$r$ (cm)'
    ...
    if plot_curr_ref == 1:
        ...
        [fig, axes] = plt.subplots(nrows=3, ncols=2, figsize=(7.5,18))
        ax1 = plt.subplot2grid( (3,1), (0,0) )
        ax2 = plt.subplot2grid( (3,1), (1,0) )
        ax3 = plt.subplot2grid( (3,1), (2,0) )
        
 
        # ax1.set_title(r"(c) $B$ (G)", fontsize = font_size,y=1.02)   
        # ax1.set_title(r"(d) $|\boldsymbol{B}|$ (G)", fontsize = font_size,y=1.02) 
        text = r'(a) $\boldsymbol{B}$ (G)'
        zstext = 0.5
        rstext = 0.95
        ax1.text(zstext,rstext,text,fontsize = text_size,color='k',ha='center',va='center',transform=ax1.transAxes) 
        ...
        # zstext = 0.5
        # rstext = 0.9
        ax2.text(zstext,rstext,text,fontsize = text_size,color='k',ha='center',va='center',transform=ax2.transAxes) 
        ...
        # ax3.set_title(r"(c)", fontsize = font_size,y=1.02)   
        text = r'(c)'
        # zstext = 0.5
        # rstext = 0.9
        ax3.text(zstext,rstext,text,fontsize = text_size,color='k',ha='center',va='center',transform=ax3.transAxes) 
    ...
    for k in range(0,nsims):
        ...
        tol_y_2           = 6*2.9/2.22
        if k == 0:
            xmin   = points[:,0].min()-tol_x_1
            # xmin   = points[:,0].min()-tol
            # xmin_1 = points[:,0].min()-tol_x_1
            xmin_2 = points[:,0].min()-tol_x_2
            # xmax   = points[:,0].max()+tol
            xmax   = points[:,0].max()+tol_x_1
            xmax_2 = points[:,0].max()+tol_x_2
            ymin   = points[:,1].min()-tol
            ymax   = points[:,1].max()+tol
            ymax_2 = points[:,1].max()+tol_y_2
    ...
    #    yticks_lim = rs[-1,-1]+1
        ...
        if plot_curr_ref == 1:
            ...
            # Plot the PIC mesh
            ax3.plot(zs_plot,rs_plot,'ko-',linewidth = line_width,markersize = marker_size)
            ax3.plot(zs_plot.transpose(),rs_plot.transpose(),'ko-',linewidth = line_width,markersize = marker_size)
            # ax1.plot(zs,rs,'ko-',linewidth = line_width,markersize = marker_size)
            # ax1.plot(zs.transpose(),rs.transpose(),'ko-',linewidth = line_width,markersize = marker_size)
            # Plot points defining mesh boundary
            if len(points) == 8:
                # Inner wall
                ax3.plot(points[0:3,0],points[0:3,1],'ro-',linewidth=line_width_boundary,markersize = marker_size)
                # Axis r = 0
                ax3.plot(points[2:4,0],points[2:4,1],'mo-',linewidth=line_width_boundary,markersize = marker_size)
                # Free loss
                ax3.plot(points[3:6,0],points[3:6,1],'bo-',linewidth=line_width_boundary,markersize = marker_size)
                # Outer wall
                ax3.plot(points[5::,0],points[5::,1],'ro-',linewidth=line_width_boundary,markersize = marker_size)
                # Injection
                ax3.plot(np.array([points[-1,0],points[0,0]]),np.array([points[-1,1],points[0,1]]),'go-',linewidth=line_width_boundary,markersize = marker_size)
            elif len(points == 10):
                # Inner wall
                ax3.plot(points[0:4,0],points[0:4,1],'ro-',linewidth=line_width_boundary,markersize = marker_size)
                # Axis r = 0
                ax3.plot(points[3:5,0],points[3:5,1],'mo-',linewidth=line_width_boundary,markersize = marker_size)
                # Free loss
                ax3.plot(points[4:7,0],points[4:7,1],'bo-',linewidth=line_width_boundary,markersize = marker_size)
                # Outer wall
                ax3.plot(points[6::,0],points[6::,1],'ro-',linewidth=line_width_boundary,markersize = marker_size)
                # Injection
                ax3.plot(np.array([points[-1,0],points[0,0]]),np.array([points[-1,1],points[0,1]]),'go-',linewidth=line_width_boundary,markersize = marker_size)
            ...
            for i in range(0,dims[0]):
                for j in range(0,dims[1]):
                    if nodes_flag[i,j] == -1:
                        if i == 0:
                            # Axis r = 0
                            ax3.plot(zs[i,j],rs[i,j],'mo',markeredgecolor='m',markersize = marker_size)
                        elif j == dims[1] - 1 or i == dims[0] - 1:
                            # Free loss
                            ax3.plot(zs[i,j],rs[i,j],'bo',markeredgecolor='b',markersize = marker_size)
                        elif j == xi_top or i == eta_max or j == xi_bottom or i == eta_min:
                            # Material wall
                            ax3.plot(zs[i,j],rs[i,j],'ro',markeredgecolor='r',markersize = marker_size)
                        elif j == 0:
                            # Injection
                            ax3.plot(zs[i,j],rs[i,j],'go',markeredgecolor='g',markersize = marker_size)
                    if nodes_flag[i,j] == 1:
                        ax3.plot(zs[i,j],rs[i,j],'ko',markersize = marker_size)      
            ax3.set_xlim(xmin,xmax)        
            ax3.set_ylim(ymin,ymax)
            # ax3.set_xticks(np.arange(0.0,xmax+1,1))
            ax3.set_xticks(np.arange(0,xmax,1))
            # ax3.set_yticks(np.arange(0.0,ymax+1,2))
            ax3.set_yticks(np.arange(0.0,ymax+1,1))
    #        ax3.set_aspect('equal', adjustable='box')
            ...
            ax2.plot(points[3:6,0],points[3:6,1],'b-',linewidth=line_width_boundary,markersize = marker_size)
            # Plot the cathode magnetic line if given
            if plot_cath_Bline == 1:
                cath_z = np.append(elem_geom[0,elems_cath_Bline_P3C],np.array([face_geom[0,cath_final_bound_face_P3C-1]]),axis=0)
                cath_r = np.append(elem_geom[1,elems_cath_Bline_P3C],np.array([face_geom[1,cath_final_bound_face_P3C-1]]),axis=0)
                ax2.plot(cath_z,cath_r,'g-',linewidth = line_width_boundary+0.5,markersize = marker_size)
                # cath_z = np.append(elem_geom[0,elems_cath_Bline_P3],np.array([face_geom[0,cath_final_bound_face_P3-1]]),axis=0)
                # cath_r = np.append(elem_geom[1,elems_cath_Bline_P3],np.array([face_geom[1,cath_final_bound_face_P3-1]]),axis=0)
                cath_z = elem_geom[0,elems_cath_Bline_P3]
                cath_r = elem_geom[1,elems_cath_Bline_P3]
                ax2.plot(cath_z,cath_r,'m-',linewidth = line_width_boundary+0.5,markersize = marker_size)
            
            # Plot the cathode position  
            ax2.plot(0.85*z_cath,r_cath,'ks',linewidth = line_width_boundary,markersize = marker_size_cath)      
                  
            # ax2.set_xlim(xmin_2,xmax_2)  
            # ax2.set_ylim(ymin,ymax_2)
            ax2.set_xlim(xmin_2,xmax)
            ax2.set_ylim(ymin,ymax)
            # ax2.set_xticks(np.arange(0.0,xmax+1,1))
            ax2.set_xticks(np.arange(0,xmax,1))
            ax2.set_yticks(np.arange(0.0,ymax+1,1))
        
            
            
            # # Plot the grey GDML region
            # z_Pinf = 1.2
            # znodes_P   = np.copy(points[5:2:-1,0])
            # rnodes_P   = np.copy(points[5:2:-1,1])
            # znodes_inf = np.copy(znodes_P)
            # rnodes_inf = np.copy(rnodes_P)
            # znodes_inf[-1] = znodes_inf[-1] + z_Pinf
            # znodes_inf[-2] = znodes_inf[-2] + z_Pinf
            # m_Plat =(rnodes_P[1]-rnodes_P[0])/(znodes_P[1]-znodes_P[0]) # Same slope at P and infinity
            # rnodes_inf[0] = rnodes_inf[0] + z_Pinf*L_c/H_c
            # rnodes_inf[1] = m_Plat*znodes_inf[1] + rnodes_inf[0]
            # znodes = np.append(znodes_P,np.flip(znodes_inf),axis=0)
            # rnodes = np.append(rnodes_P,np.flip(rnodes_inf),axis=0)
            # znodes = np.append(znodes,np.array([znodes_P[0]]),axis=0)
            # rnodes = np.append(rnodes,np.array([rnodes_P[0]]),axis=0)
            # ax2.fill(znodes, rnodes, color = silver) 
            
            
            # text = 'GDML'
            # zstext = 6
            # rstext = 13.2
            # ax2.text(zstext,rstext,text,fontsize = text_size-5,color='k',ha='center',va='center')   
            text = r'$\phi_\infty, I_\mathrm{P} = 0$'
            # zstext = 11
            # rstext = 16.2
            zstext = 2.8
            rstext = 12.3
            ax2.text(zstext,rstext,text,fontsize = text_size-5,color='b',ha='center',va='center') 
            text = r'$V_\mathrm{d}$'
            zstext = -1.7
            rstext = 4.25
            ax2.text(zstext,rstext,text,fontsize = text_size-5,color='k',ha='center',va='center') 
            ...
            # ax2.annotate('Plume boundary P', xy=(4, 11), xytext=(-1, 14),
            ax2.annotate('Plume boundary P', xy=(6, 12.4), xytext=(1, 13),
            arrowprops=dict(color='blue',facecolor='blue', shrink=0.05,width=line_width_boundary,headwidth=4.5*line_width_boundary,headlength=5.5*line_width_boundary),
            fontsize=text_size-5,color='blue')
            ax2.annotate('', xy=(-0.5, 5.41), xytext=(0.5, 5.41),
            arrowprops=dict(color='red',facecolor='red', shrink=0.05,width=line_width_boundary+0.5,headwidth=4.5*line_width_boundary,headlength=5.5*line_width_boundary),
            fontsize=text_size-5)
            ax2.annotate('Cathode', xy=(0.9, 5.8), xytext=(-1.5, 9),
            arrowprops=dict(color='black',facecolor='black', shrink=0.05,width=line_width_boundary,headwidth=4.5*line_width_boundary,headlength=5.5*line_width_boundary),
            fontsize=text_size-5,color='black')
            
            # Draw the circuit
            # Line from anode
            ax2.plot(np.array([0.0,-1.0,-1.0]),np.array([2.955,2.955,4.0]),'k-',linewidth = line_width_boundary+0.5,markersize = marker_size)
            # Line from cathode
            ax2.plot(np.array([1.0,-1.0,-1.0]),np.array([5.41,5.41,4.5]),'k-',linewidth = line_width_boundary+0.5,markersize = marker_size)
            # Power source negative side
            ax2.plot(np.array([-1.1,-0.9]),np.array([4.5,4.5]),'k-',linewidth = line_width_boundary+0.5,markersize = marker_size)
            # Power source positive side
            ax2.plot(np.array([-1.3,-0.7]),np.array([4.0,4.0]),'k-',linewidth = line_width_boundary+0.5,markersize = marker_size)
    ...
    #        lines_ticks_loc  =  [(10.6,0.37),(10.02,2.21),(9.0,4.25),(7.68,4.25),(0.17,4.2),(0.7, 4.4),(1.7, 4.25),(2.5, 3.7),(5.8,4.25),(4.5,4.25),(3.2,4.25)]
            ...
            lines_ticks_color = 'k'
            lines_style       = '-'
            [CS,CS2] = contour_2D (ax1,'$z/L_\mathrm{c}$', '$r/H_\mathrm{c}$', font_size, ticks_size, zs, rs, Bfield_plot, nodes_flag, log_type, auto, 
            ...
            # ax1.clabel(CS2, CS2.levels, fmt=fmt_func_exponent_lines, inline=1, fontsize=ticks_size_isolines)
    #        ax1.clabel(CS2, CS2.levels, fmt=fmt_func_exponent_lines, inline=1, manual = lines_ticks_loc, fontsize=ticks_size_isolines, zorder = 1)
    ...
    #        nstart_z = 7
            ...
            # streamline_color = 'b'
            # arrow_size = 1
            stream = streamline_2D(ax1,zvec,rvec,dims,zs,rs,Bz_plot,Br_plot,flag_start,start_points,
                                   plot_start_points,streamline_density,streamline_width,streamline_color,arrow_size,
                                   arrow_style,min_length)
            
            ax1.set_xlim(xmin,xmax)        
            ax1.set_ylim(ymin,ymax)
            ax1.set_yticks(np.arange(0.0,ymax+1,1))
            # ax1.set_xticks(np.arange(0,xmax+1,1))
            ax1.set_xticks(np.arange(0,xmax,1))
            
            # Plot singular magnetic line if required
            if plot_B0_line == 1:
                ax1.plot(B0_line_nodez,B0_line_noder,color=B0_line_color,linestyle=B0_line_linestyle,linewidth = line_width_boundary+0.5,markersize = marker_size)
                ax1.plot(B0_line_nodez[0],B0_line_noder[0],color=B0_line_color,linestyle=B0_line_linestyle,linewidth = line_width_boundary+0.5,marker='o',markersize = marker_size_cath-2)
            ...
            plt.tight_layout()
            if save_flag == 1:
                plt.savefig(path_out+"fig1"+figs_format,bbox_inches='tight')
                plt.savefig(path_out+"fig1"+figs_format2,bbox_inches='tight')
                plt.close()
