...
#    plot_cath_contours = 0
    ...
    plot_cath_Bline = 1
    cath_Bline_color = ['darkred','darkolivegreen','c']
   ...
   # Cathode final boundary face for cases P2_fcat3198
    ...
    # cath_final_bound_face = [cath_final_bound_face_P2,cath_final_bound_face_P2,
    #                          cath_final_bound_face_P3,cath_final_bound_face_P3,
    #                          cath_final_bound_face_P2,cath_final_bound_face_P2,
    #                          cath_final_bound_face_P3,cath_final_bound_face_P3]
    cath_final_bound_face = [cath_final_bound_face_P4,cath_final_bound_face_P3,cath_final_bound_face_P2]
    cath_final_bound_face = []
    
    # Elements along cathode B line for VHT_US P20 C1 cases
    elems_cath_Bline_P2 = list(range(2393-1,2393-1-46,-1))  
    # Elements along cathode B line for VHT_US P30 C1 cases
    ...
    # Elements along cathode B line for VHT_US P40 C1 cases
    elems_cath_Bline_P4 = list(range(1579-1,1579-1-80,-2))   
    ...
    elems_cath_Bline_P2_C2 = [1978, 1976, 1974, 1972, 1970, 1968, 1966, 1963, 1962, 1960,
    ...
    elems_cath_Bline_P3_C2 = [1289, 1287, 1285, 1283, 1281, 1279, 1277, 1275, 1272, 1270,
    ...
    elems_cath_Bline_P4_C2 = [4023, 4022, 4021, 4020, 4019, 4018, 4017, 4016, 4015, 4014,
    ...
    #                    B0_face_axis_ID_P3,B0_face_axis_ID_P3]
    B0_face_axis_ID = [B0_face_axis_ID_P4,B0_face_axis_ID_P3,B0_face_axis_ID_P2]
...
#    last_steps      = 1000
    ...
    vline = 1         # Draw a vertical line on P3 and P4 corresponding to vertical P boundary of P2 and P3, respectively
    color_vline = 'w'
...
#    cont_ylabel = '$r$ (cm)'
    ...
    #    nu_i12_plot       = nu_i12_plot*1E-6
        ...
        # C2 cases: set cathode elements depending on case
        if k == 0:
            elems_cath_Bline_C2 = elems_cath_Bline_P4_C2
        elif k == 1:
            elems_cath_Bline_C2 = elems_cath_Bline_P3_C2
        elif k == 2:
            elems_cath_Bline_C2 = elems_cath_Bline_P2_C2
    ...
    #    yticks_lim = rs[-1,-1]+1
        ...
        if plot_curr_ref == 1:
            ...
            plot_MFAM_ax_nosig(ax,faces,nodes,line_width,colors[k])
            ...
            for i in elems_cath_Bline:
                ...
                ax.fill(znodes, rnodes, color = colors[k],alpha=0.5) 
                # ax.fill(znodes, rnodes, color = silver) 
            for i in elems_cath_Bline_C2:
                nnodes = elem_n[0,i]
                nodes_ID = elem_n[1:nnodes+1,i] - 1
                znodes = nodes[0,nodes_ID]
                rnodes = nodes[1,nodes_ID]
            ...
            if plot_cath_Bline == 1:
                if np.any(cath_final_bound_face) != 0:
                ...
                plt.plot(cath_z,cath_r,color=cath_Bline_color[k],linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
                
                if np.any(cath_final_bound_face) != 0:
                    cath_z_C2 = np.append(elem_geom[0,elems_cath_Bline],np.array([face_geom[0,cath_final_bound_face[k]-1]]),axis=0)
                    cath_r_C2 = np.append(elem_geom[1,elems_cath_Bline],np.array([face_geom[1,cath_final_bound_face[k]-1]]),axis=0)
                else:
                    cath_z_C2 = elem_geom[0,elems_cath_Bline_C2]
                    cath_r_C2 = elem_geom[1,elems_cath_Bline_C2]
                plt.plot(cath_z_C2,cath_r_C2,color=cath_Bline_color[k],linestyle=cath_Bline_linestyle,linewidth = line_width_boundary,markersize = marker_size)
            
             # Plot singular magnetic line if required
            if plot_B0_line == 1:
                plt.plot(B0_line_nodez,B0_line_noder,color=B0_line_color[k],linestyle=B0_line_linestyle,linewidth = line_width_boundary,markersize = marker_size)
            ...
            # Plot the thruster channel midline
